...
import { ArrowUpDown, Calendar, Download, MoreHorizontal, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
function exportAcademicYears(rows) {
    ...
    const body = rows.map((row) => {
        ...
        return [a.name, a.startDate || a.start_date, a.endDate || a.end_date, a.status];
    });
    ...
    anchor.href = url;
    anchor.download = 'ocms-academic-years.csv';
    anchor.click();
    URL.revokeObjectURL(url);
}

function AcademicYearsDataTable({ data, onDelete }) {
        ...
        },
        {
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem asChild>
                <Link to={`/academic-years/${row.original.id}/edit`}>
                  <Pencil />
                  Edit
                </Link>
              </DropdownMenuItem>
    ...
    return (<div className="space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative max-w-sm">
          ...
          <Input className="pl-9" placeholder="Search academic years..." value={globalFilter ?? ''} onChange={(event) => setGlobalFilter(event.target.value)}/>
        </div>

        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={() => exportAcademicYears(table.getFilteredRowModel().rows)}>
            <Download />
            Export
          </Button>
          <Button asChild>
            <Link to="/academic-years/add">
              <Plus />
              Add Academic Year
            </Link>
          </Button>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (table.getRowModel().rows.map((row) => (<TableRow key={row.id}>
                ...
                <TableCell colSpan={columns.length} className="p-6">
                  <EmptyState title="No academic years found" description="Create an academic year to get started." actionLabel="Add Academic Year" actionTo="/academic-years/add"/>
                </TableCell>
              </TableRow>)}
          </TableBody>
        </Table>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted-foreground">
          Showing {table.getRowModel().rows.length} of {table.getFilteredRowModel().rows.length} academic years
        </p>
...
export function AcademicYearsList() {
    ...
    const fetchAcademicYears = useCallback(() => {
        ...
        getAcademicYears()
            ...
            .catch((err) => setError(err.message || 'Failed to load academic years.'))
    ...
    const handleDelete = useCallback(async () => {
        ...
        try {
            ...
            setAcademicYears((prev) => prev.filter((a) => a.id !== deleteTarget.id));
        }
        catch (err) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-normal sm:text-3xl">Academic Years</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {loading ? 'Loading...' : `${academicYears.length} academic year${academicYears.length !== 1 ? 's' : ''} registered`}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Academic Year Directory</CardTitle>
          <CardDescription>Search, filter, paginate, export, and manage academic year records.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
